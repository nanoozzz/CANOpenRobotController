    ...
    bool required)
{
    ...
    if (helper_ == nullptr) {
        spdlog::error(
            "M2FittsHuman/UI: received null FLNLHelper.");
        return false;
    }

    spdlog::info(
        "M2FittsHuman/UI: using existing FLNL server/helper. "
...
void M2FittsUILink::update()
{
    ...
    if (isConnected && !wasConnected_) {
        ...
        spdlog::info(
            "M2FittsHuman/UI: client connected.");
        ...
        spdlog::warn(
            "M2FittsHuman/UI: client disconnected.");
    ...
    if (!isConnected) {
        ...
        // M2FittsHumanMachine/UIserver is the owner of the
        // FLNL server.
        //
...
{
    spdlog::info(
        "M2FittsHuman/UI: client connection detected.");
...
void M2FittsUILink::processIncoming()
{
    ...
    spdlog::debug(
        "M2FittsHuman/UI: received command '{}' with {} parameters.",
        cmd,
        p.size());
    ...
    if (cmd == "HELO") {
        ...
        if (std::lround(clientVersion)
            ...
            spdlog::critical(
                "M2FittsHuman/UI: protocol mismatch. "
                "Client={}, Server={}.",
                clientVersion,
                M2FITTS_PROTOCOL_VERSION);
        ...
        spdlog::info(
            "M2FittsHuman/UI: handshake completed "
            "(protocol v{}).",
            M2FITTS_PROTOCOL_VERSION);
    ...
    spdlog::warn(
        "M2FittsHuman/UI: unknown command '{}'.",
        cmd);
}
